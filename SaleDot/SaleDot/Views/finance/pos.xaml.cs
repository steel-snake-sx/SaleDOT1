    ...
    public partial class pos : Window
    {
        ...
        private void refreshCartAndTotal()
        {
            try
            {
                ...
                foreach (productsaleorpurchaseviewmodel item1 in cart)
                {
                    ...
                    lastInsertedProductInCart = item1;
                    var sql = "SELECT quantity FROM saledot.product where barcode = " + item1.barcode + ";";
                    using (var connection = new MySqlConnection(conn))
                    {
                        double res = connection.Query<double>(sql).FirstOrDefault();
                        if (item1.quantity > res)
                        {
                            MessageBox.Show("Товара на складе меньше чем вы добавили в корзину. Остаток: " + res, "Информация");
                            item1.quantity = 1;
        ...
        void doneSale()
        {
            try
            {
                ...
                MessageBox.Show("Сумма продажи: " + totalbill, "Успех");
                Close();
                new pos().Show();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Продажа не сохранена \n" + ex.Message, "Информация");
            }
